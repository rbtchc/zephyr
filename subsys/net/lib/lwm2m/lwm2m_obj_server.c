...
static u8_t  transport_binding[MAX_INSTANCE_COUNT][TRANSPORT_BINDING_LEN];

static struct lwm2m_engine_obj server;
static struct lwm2m_engine_obj_field fields[] = {
	OBJ_FIELD_DATA(SERVER_SHORT_SERVER_ID, R, UINT),
	OBJ_FIELD_DATA(SERVER_LIFETIME_ID, RW, UINT),
	OBJ_FIELD_DATA(SERVER_DEFAULT_MIN_PERIOD_ID, RW, UINT),
	OBJ_FIELD_DATA(SERVER_DEFAULT_MAX_PERIOD_ID, RW, UINT),
	OBJ_FIELD_EXECUTE(SERVER_DISABLE_ID),
	OBJ_FIELD_DATA(SERVER_DISABLE_TIMEOUT_ID, RW, UINT),
	OBJ_FIELD_DATA(SERVER_STORE_NOTIFY_ID, RW, UINT),
	/* Mark Transport Binding RO as we only support UDP atm */
	OBJ_FIELD_DATA(SERVER_TRANSPORT_BINDING_ID, R, UINT),
...
static struct lwm2m_engine_obj_inst *server_create(u16_t obj_inst_id)
{
	...
	INIT_OBJ_RES_DATA(res[index], i, SERVER_TRANSPORT_BINDING_ID,
		transport_binding, TRANSPORT_BINDING_LEN);
...
static int lwm2m_server_init(struct device *dev)
{
	...
	engine_register_obj(&server);

	/* auto create the first instance */
	ret = engine_create_obj_inst(LWM2M_OBJECT_SERVER_ID, 0, obj_inst);
	if (ret < 0) {
