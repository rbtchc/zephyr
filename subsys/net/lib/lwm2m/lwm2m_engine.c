...
int lwm2m_engine_create_obj_inst(char *pathstr)
{
	...
	return engine_create_obj_inst(path.obj_id, path.obj_inst_id,
				      &obj_inst);
}
		      ...
		      struct lwm2m_engine_context *context)
{
	...
	while (obj_inst) {
		...
		for (index = 0; index < obj_inst->resource_count; index++) {
			 ...
			 * for engine_read_handler to read this specific
			 * resource.
			 */
			  ...
			  struct sockaddr *from_addr)
{
	...
	case LWM2M_OP_WRITE_ATTR:
		r = engine_write_attr_handler(obj, &context);
		break;

	case LWM2M_OP_EXECUTE:
		r = engine_exec_handler(obj, &context);
		break;

	case LWM2M_OP_DELETE:
		r = engine_delete_handler(obj, &context);
		break;
				   ...
				   bool manual_trigger)
{
	...
	ret = zoap_pending_init(pending, out.out_zpkt, &obs->addr);
	if (ret < 0) {
...
static int lwm2m_engine_init(struct device *dev)
{
	...
	k_thread_create(&engine_thread_data,
			...
			K_THREAD_STACK_SIZEOF(engine_thread_data),
