...
#if 0
static int zoap_init_message(struct zoap_packet *zpkt, struct net_pkt **pkt,
		  u8_t type, u8_t code, u16_t mid, const u8_t *token, u8_t tkl,
			      ...
			      const struct sockaddr *from)
{
	...
	if (payload_len > 0) {
		...
		/* callback */
		callback = lwm2m_firmware_get_rw_cb();
		if (callback) {
