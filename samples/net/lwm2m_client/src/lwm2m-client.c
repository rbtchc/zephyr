...
{
	SYS_LOG_INF("FIRMWARE: BLOCK RECEIVED");
	return 1;
}
...
static int lwm2m_setup(void)
{
	...
	lwm2m_engine_register_write_callback("5/0/0",
					     firmware_block_received_cb);
	lwm2m_firmware_set_rw_cb(firmware_block_received_cb);
...
void main(void)
{
	...
	ret = set_endpoint_name(ep_name, udp6.local.family);
	if (ret < 0) {
	...
	ret = lwm2m_engine_start(udp6.ipv6_ctx);
	if (ret < 0) {
		SYS_LOG_ERR("Cannot init LWM2M IPv6 engine (%d)", ret);
		goto cleanup_ipv6;
	}

	ret = lwm2m_rd_client_start(udp6.ipv6_ctx, &udp6.remote,
				    ep_name);
	if (ret < 0) {
	...
	ret = set_endpoint_name(ep_name, udp4.local.family);
	if (ret < 0) {
		SYS_LOG_ERR("Cannot set IPv4 endpoint name (%d)", ret);
		goto cleanup_ipv4;
	}

	ret = lwm2m_engine_start(udp4.ipv4_ctx);
	if (ret < 0) {
		SYS_LOG_ERR("Cannot init LWM2M IPv4 engine (%d)", ret);
		goto cleanup_ipv4;
	}

	ret = lwm2m_rd_client_start(udp4.ipv4_ctx, &udp4.remote,
				    ep_name);
	if (ret < 0) {
